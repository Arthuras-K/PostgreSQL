...
SELECT genre.name, COUNT(*) FROM genre 
       JOIN genre_mus ON genre.id = genre_mus.genre_id
       JOIN musician ON genre_mus.musician_id = musician.id
GROUP BY genre.name
...
SELECT album.name, ROUND(AVG(time),2) FROM track 
       JOIN album ON track.album_id = album.id
GROUP BY album.name
ORDER BY album.name;
...
SELECT DISTINCT musician.name FROM album 
       JOIN album_mus ON album.id = album_mus.album_id
       JOIN musician ON album_mus.musician_id = musician.id
...
SELECT DISTINCT collection.name FROM collection 
       JOIN track_coll ON collection.id = track_coll.collection_id
       JOIN track ON track_coll.track_id = track.id
WHERE track.name IN (
     SELECT track.name FROM musician 
       JOIN album_mus ON musician.id = album_mus.album_id 
        AND musician.name = 'Fever 333'
       JOIN album ON album_mus.album_id = album.id
...
SELECT album.name FROM musician 
 ...
 WHERE musician.name IN (
       SELECT musician.name FROM musician 
         JOIN genre_mus ON musician.id = genre_mus.musician_id
         JOIN genre ON genre_mus.genre_id = genre.id
...
SELECT track.name FROM track 
 WHERE track.name NOT IN (
       SELECT DISTINCT track.name FROM track
         JOIN track_coll ON track.id = track_coll.track_id
         JOIN collection ON track_coll.collection_id = collection.id);



/* исполнителя(-ей), написавшего самый короткий по продолжительности трек (теоретически таких треков может быть несколько) */
SELECT DISTINCT musician.name, time FROM musician 
  JOIN album_mus ON musician.id = album_mus.musician_id 
  JOIN album ON album_mus.album_id = album.id
  JOIN track ON track.album_id = album.id
